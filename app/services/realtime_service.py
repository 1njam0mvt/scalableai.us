...
class RealtimeGroqService(GroqService):
    ...
    def _extract_search_query(
        ...
        try:
            ...
            if isinstance(raw_content, list):
                extracted = " ".join(str(item) for item in raw_content)
            else:
                extracted = str(raw_content)
    ...
    def search_tavily(self, query: str, num_results: int = 5) -> Tuple[str, Optional[dict]]:
        ...
        try:
            t0 = time.perf_counter()
            # assign to local var so static analyzers know it's not None
    ...
    def stream_response(self, question: str, chat_history: Optional[List[tuple]] = None, key_start_index: int = 0, extra_system_parts: Optional[List[str]] = None) -> Iterator[Any]:
        ...
        try:
            ...
            formatted_results, payload = self.search_tavily(search_query, num_results=5)
            # payload may be a string or a dict; ensure it's a dict before calling .get
            num_results = len(payload.get("results", [])) if isinstance(payload, dict) else 0
            ...
            combined_parts = list(extra_system_parts) if extra_system_parts else []
            if formatted_results:
                combined_parts.append(escape_curly_braces(formatted_results))
            prompt, messages = self._build_prompt_and_messages(
                question, chat_history,
                extra_system_parts=combined_parts or None,
                mode_addendum=REALTIME_CHAT_ADDENDUM,
            )
