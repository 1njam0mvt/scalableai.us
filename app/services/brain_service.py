...
**task** — User wants ONLY an ACTION performed (no question to answer). Opening apps/websites, playing music/video, generating images, writing content, searching Google/YouTube/site-specific search, calculating math, setting reminders, or controlling the webcam.
Examples: "Open YouTube" / "Play despacito" / "Generate image of a cat" / "Write an essay about AI" / "Search for Python tutorials" / "Open webcam" / "Close webcam" / "Launch Netflix" / "Go to Facebook" / "Make me a picture of a sunset" / "Draw a cat" / "Create an image of mountains" / "What is 45 times 12?" / "Calculate 200 divided by 8" / "Search for headphones on Amazon" / "Look up quantum computing on Wikipedia" / "Remind me to call mom in 10 minutes" / "Set a reminder to check the oven in 20 minutes"
- ANY request to open, launch, play, generate, draw, create, write, draft, compose, search, calculate, remind, or control webcam → task
- "Open webcam" / "Turn on camera" / "Close webcam" / "Turn off camera" → task
- Image/picture/drawing requests → task (NOT camera)
- Pure arithmetic ("what is X [+-*/] Y", "calculate ...") → task (NOT general — general is for word problems/explanations, task is for direct computation)
- "Remind me to X in/after Y" / "set a reminder" / "set a timer" → task
...
_TASK_BRAIN_PROMPT = """You are a very accurate Decision-Making Model for SCALABLE. You decide what kind of task the user wants and extract the CLEAN query/topic for each task.
...
-> 'open (website/app name)' — Open a website or app.
   ...
   "Open github.com/1njam0mvt for everyone" → open github.com/1njam0mvt
...
-> 'play (song/video name)' — Play music or video on YouTube.
   ...
   "Hello Scalable can you play Shape of You on YouTube" → play Shape of You
   "Hey Scalable Teja song Dhurandhar title track can you play that on YouTube" → play Teja Dhurandhar title track
...
-> 'google_search (search topic)' — Search something on Google.
   ...
   "Hey Scalable look up best restaurants nearby" → google_search best restaurants nearby
...
-> 'calculate (math expression)' — Compute an arithmetic expression. Keep the expression in a form usable with digits/operators.
   "What is 45 times 12?" → calculate 45 * 12
   "Calculate 200 divided by 8" → calculate 200 / 8
   "How much is 15% of 300?" → calculate 300 * 0.15
   "What's 7 plus 3 minus 2?" → calculate 7 + 3 - 2

-> 'site_search (site_name|search terms)' — Search a specific named site (Amazon, Wikipedia, Google Maps, Flipkart, eBay). Format: site name, a pipe character, then the query.
   "Search for wireless headphones on Amazon" → site_search amazon|wireless headphones
   "Look up quantum computing on Wikipedia" → site_search wikipedia|quantum computing
...
-> 'reminder (full reminder request)' — Set a reminder/timer. Keep the ENTIRE original phrase including the time duration, so it can be parsed later.
   ...
   "Set a timer for 5 minutes" → reminder set a timer for 5 minutes
...
=== CRITICAL RULES ===
*** Extract ONLY the relevant topic/query — REMOVE greetings (hello, hey, hi), assistant name (Scalable), filler words (can you, please, for me), platform names (on YouTube, on Google), and command words from the query. ***
...
class BrainService:
    ...
    _TASK_FEW_SHOTS = [
        ...
        ("hello Scalable can you play Shape of You", "play Shape of You"),
        ...
        ("hey Scalable Teja song Dhurandhar title track can you play that on YouTube", "play Teja Dhurandhar title track"),
        ("can you open the website Scalable for everyone", "open jarvisforeveryone.com"),
        ...
        if decisions:
            for task_type, clean_query in decisions:
                ...
                if task_type == "open":
                    ...
                    payload["expression"] = clean_query or user_message
                elif task_type == "site_search":
                    if "|" in clean_query:
                        site, _, query = clean_query.partition("|")
                    ...
                    payload["message"] = clean_query or user_message
...
Classify. Output EXACTLY ONE category name."""
    ...
    def _parse_task_decisions(self, raw_response: str) -> List[Tuple[str, str]]:
        ...
        return decisions if decisions else [("unknown", "")]
    ...
    def _rule_based_primary(self, msg: str) -> str:
        ...
        task_patterns = [
            ...
            "remind me", "set a reminder", "set a timer",
        ]

        if any(m.startswith(p) or p in m for p in task_patterns):

            # "what is"/"what's" is ambiguous (could be a general knowledge
            # question) - only treat as task if it looks like arithmetic.
            if (m.startswith("what is ") or m.startswith("what's ")) and not re.search(r"\d", m):
                pass  # fall through to realtime/general handling below
            else:
                return "task"
    ...
    def _rule_based_task(self, msg: str) -> List[str]:
        ...
        Must return strings in "task_type query" form (same shape the LLM
        produces) since _parse_task_decisions splits the query back out of
        each string. Returning a bare task type here (e.g. "open" with no
        site name) silently drops the query and every task falls back to
        its default (e.g. "open gmail" opening Google instead of Gmail).
        """
        ...
        if m.startswith(("open ", "launch ", "go to ", "visit ", "can you open ")) or \
           ("open" in m and any(s in m for s in ["facebook", "youtube", "google", "netflix", "gmail", "instagram", "twitter", "linkedin"])):
            ...
            tasks.append(f"open {query}".strip())

        if m.startswith(("play ", "play the ", "play some ")) or " play " in m:
            query = self._extract_play_query(msg)
            tasks.append(f"play {query}".strip())

        if any(x in m for x in ["generate image", "draw ", "create image", "make me ", "picture of ", "image of "]):
            query = self._extract_image_prompt(msg)
            tasks.append(f"generate_image {query}".strip())

        if any(x in m for x in ["write ", "draft ", "compose ", "essay", "poem", "letter", "application "]):
            query = self._extract_content_prompt(msg)
            tasks.append(f"content {query}".strip())

        if "youtube" in m and any(x in m for x in ["search", "find"]):
            query = self._extract_search_query(msg)
            tasks.append(f"youtube_search {query}".strip())

        if any(x in m for x in ["search for ", "look up ", "find me ", "google "]) and "youtube" not in m:
            site_keywords = ["amazon", "wikipedia", "flipkart", "ebay", "google maps"]

            if any(s in m for s in site_keywords):
                site, query = self._extract_site_search(msg)
                tasks.append(f"site_search {site}|{query}".strip())
            else:
                query = self._extract_search_query(msg)
        ...
        if any(x in m for x in ["remind me", "set a reminder", "set a timer"]):
            tasks.append(f"reminder {msg}".strip())

        if tasks:
            return tasks

        urls = self._extract_urls(msg)
        return [f"open {urls[0]}".strip()] if urls else ["unknown"]
    ...
    def _strip_filler(self, msg: str) -> str:
        ...
        cleaned = re.sub(
            r'^(?:hello|hi|hey|yo|hiya|howdy|ok|okay|alright)\s+(?:scalable|s\.?c\.?a\.?l\.?a\.?b\.?l\.?e\.?)\s*[,.]?\s*',
            '', cleaned, flags=re.I
        ).strip()
        ...
        cleaned = re.sub(
            r'^(?:scalable|s\.?c\.?a\.?l\.?a\.?b\.?l\.?e\.?)\s*[,.]?\s*',
            '', cleaned, flags=re.I
        ).strip()
    ...
    def _extract_site_search(self, msg: str) -> tuple:
        ...
        LLM's structured 'site_search site|query' output isn't available."""

        m = self._strip_filler(msg)
        m_lower = m.lower()

        known_sites = ["amazon", "wikipedia", "flipkart", "ebay", "google maps", "maps"]
        site = ""

        for s in known_sites:
            if s in m_lower:
                site = "maps" if s == "google maps" else s
                break

        # Remove the site name and common connector words to isolate the query.
        query = re.sub(
            r"\b(on|in|from|search|for|find|look up|look for|"
            + "|".join(known_sites) + r")\b",
            "", m, flags=re.I,
        )
        query = re.sub(r"\s+", " ", query).strip(" .,!?")
        return (site, query or m)
    ...
    def _extract_payload(self, task_type: str, message: str):
        ...
        if task_type == "play":
            ...
            site, query = self._extract_site_search(message)
            payload["site"] = site
            payload["query"] = query
    ...
    def _extract_image_prompt(self, msg: str) -> str:
        ...
        # Longest/most specific prefixes first, so "image of X" matches
        # before the bare "generate " prefix would eat into X's own text
        # (e.g. a subject that itself contains the word "generate").
        prefixes = ["generator image of ", "picture of ", "image of ",
                    "make me a ", "make me an ", "make me ", "make a ", "make an ",
                    "draw me a ", "draw me an ", "draw me ",
                    "generate ", "draw ", "create "]

        for p in prefixes:
            idx = lower.find(p)
            if idx != -1:
                # Strip everything up to and including this ONE prefix match -
                # deliberately not repeated, so a second occurrence of a
                # trigger word (e.g. "image of generate X") stays part of the subject.
                m = m[idx + len(p):].strip()
                extracted = m
                break
        ...
        # Strip a single leading article left over from phrases like
        # "generate " + "a picture of" -> "a cat" (keep the noun phrase natural)
        m2 = re.sub(r"^(?:a|an)\s+", "", extracted, count=1, flags=re.I)
